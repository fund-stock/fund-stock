...
func MinNum(num float64) float64 {
	arr := strings.Split(fmt.Sprintf("%v", num), ".")
	zs := StrToFloat64(arr[0])
	xs := StrToFloat64(arr[1])
	pf := math.Pow(10, float64(len(arr[1])))
	if zs == 0 {
		pf = math.Pow(10, float64(len(arr[1])+1))
		return xs / pf
	}
	if pf == 0 {
		return 0
