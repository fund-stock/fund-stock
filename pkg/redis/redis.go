...
func (rdb *Redis) SAdd(key string, members ...interface{}) int64 {
	return rdb.Client.SAdd(Set(key), members...).Val()
}

func (rdb *Redis) SRem(key string, members ...interface{}) int64 {
	return rdb.Client.SRem(Set(key), members...).Val()
}

func (rdb *Redis) SCard(key string) int64 {
	return rdb.Client.SCard(Set(key)).Val()
}

func (rdb *Redis) ZRangeWithScores(key string, start, stop int64) []redis.Z {
	return rdb.Client.ZRangeWithScores(Set(key), start, stop).Val()
}

func (rdb *Redis) ZRem(key string, members ...interface{}) int64 {
	return rdb.Client.ZRem(Set(key), members...).Val()
}
...
func (rdb *Redis) Rpush(key string, values ...interface{}) (bool, error) {
	_, err := rdb.Client.RPush(Set(key), values...).Result()
	if err != nil {
...
func (rdb *Redis) SelectDbSetNx(db int, key string, value interface{}, exTime int) (bool, error) {
	defer rdb.mutex.Unlock()
	rdb.mutex.Lock()
	pipe := rdb.Client.Pipeline()
	pipe.Select(db)
	...
	pipe.SetNX(key, value, time.Duration(rdb.expireTime)*time.Second)
	pipe.Select(rdb.DefaultDB)
	cmderArr, err := pipe.Exec()
	strMap := GetCmdResult(cmderArr)
	if strMap[1].(bool) {
		return true, err
