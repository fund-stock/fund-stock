...
kill -9 $app_pid7
sleep 3
app_pid8=$(ps -ef | grep $runname8 | grep 07 | awk '{print $2}')
...
if [ $app_new_pid1 -gt 0 ]; then
  ...
  echo -e "$date $app已更新 " | mail -v -s '重启服务通知' 'yezhiming@yuctime.com' >/dev/null 2>&1
else
  echo "$runname1运行失败，请检查原因"
  echo "$date $runname1运行失败，请检查原因" >>$appdirLogs/update.log
  echo -e "$date $app更新失败，请检查原因 " | mail -v -s '重启服务通知' 'yezhiming@yuctime.com' >/dev/null 2>&1
  exit 0
fi
